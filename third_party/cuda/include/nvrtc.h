 ...
 *          of an actual architecture. The cubin does not contain code for the
 *          Tile functions (\c __tile__ / \c __tile_global__) or variables (\c __tile__);
 *          use \c nvrtcGetTileIR() to extract the cuda_tile IR generated for Tile
 *          code. 
 *
 * \param   [in]  prog CUDA Runtime Compilation program.
...
/**
 * \ingroup compilation
 * \brief   nvrtcGetTileIRSize sets the value of \p TileIRSizeRet with the size of the cuda_tile IR
 *          generated by the previous compilation of \p prog. 
 *
 * \param   [in]  prog       CUDA Runtime Compilation program.
 * \param   [out] TileIRSizeRet Size of the generated cuda_tile IR.
 * \return
 *   - \link #nvrtcResult NVRTC_SUCCESS \endlink
 *   - \link #nvrtcResult NVRTC_ERROR_INVALID_INPUT \endlink
 *   - \link #nvrtcResult NVRTC_ERROR_INVALID_PROGRAM \endlink
...
/**
 * \ingroup compilation
 * \brief   nvrtcGetTileIR stores the cuda_tile IR generated by the previous compilation
 *          of \p prog in the memory pointed by \p TileIR. 
 *
 * \param   [in]  prog CUDA Runtime Compilation program.
 * \param   [out] TileIR  Generated cuda_tile IR.
 * \return
 *   - \link #nvrtcResult NVRTC_SUCCESS \endlink
 *   - \link #nvrtcResult NVRTC_ERROR_INVALID_INPUT \endlink
 *   - \link #nvrtcResult NVRTC_ERROR_INVALID_PROGRAM \endlink
 *
 * \see     ::nvrtcGetTileIRSize
 */
nvrtcResult nvrtcGetTileIR(nvrtcProgram prog, char* TileIR);

/**
 * \defgroup bundled_headers Bundled Headers Installation
 *
 * NVRTC defines the following types and functions for bundled headers
 ...
 * \brief Flags for nvrtcInstallBundledHeaders
 */
/**
 * \ingroup bundled_headers
 * \brief Skip installation if version marker exists and version matches.
 *        This is the default behavior when flags=0.
 */
#define NVRTC_INSTALL_HEADERS_SKIP_IF_EXISTS     0x0
/**
 * \ingroup bundled_headers
 * \brief Clear existing directory contents before installation.
 *        Guarantees consistency by removing any existing files first.
 */
#define NVRTC_INSTALL_HEADERS_FORCE_OVERWRITE    0x1
/**
 * \ingroup bundled_headers
 * \brief Return NVRTC_ERROR_BUSY immediately if installation is in progress
 *        by another process, instead of waiting for the lock.
 *        Can be combined with FORCE_OVERWRITE using bitwise OR.
 */
#define NVRTC_INSTALL_HEADERS_NO_WAIT            0x2

/**
 * \ingroup bundled_headers
 * \brief   nvrtcInstallBundledHeaders extracts CUDA headers bundled with NVRTC
 *          to a specified directory for use during compilation.
 *
 * NVRTC bundles a set of CUDA Toolkit headers and CUDA C++ Core Libraries (CCCL)
 * within libnvrtc-builtins. This function extracts these headers to the specified
 * directory, allowing NVRTC programs to compile without requiring a separate CUDA
 * Toolkit installation. The bundled headers match those available in the CUDA Toolkit
 * plus CCCL libraries.
 *
 * After extraction, users can compile kernels by passing appropriate include paths
 * (such as "-I<installPath>" and "-I<installPath>/cccl") to nvrtcCompileProgram.
 *
 * A version marker file (.nvrtc_headers_version) is created in the installation
 * directory to track the installed version.
 *
 * This function is thread-safe and process-safe. Concurrent calls from multiple
 * threads or processes will be serialized using file locking. By default, the
 * function waits for the lock; use NVRTC_INSTALL_HEADERS_NO_WAIT to return
 * immediately with NVRTC_ERROR_BUSY if another process holds the lock.
 *
 * \note    Use NVRTC_INSTALL_HEADERS_SKIP_IF_EXISTS to avoid reinstalling if headers
 *          already exist. Use NVRTC_INSTALL_HEADERS_FORCE_OVERWRITE to guarantee
 *          consistency by clearing the directory first.
 *
 * \param   [in] installPath Path where headers should be extracted (UTF-8 encoded).
 *                           The directory will be created if it doesn't exist.
 * \param   [in] flags       NVRTC_INSTALL_HEADERS_* flags:
 *                           - 0 (SKIP_IF_EXISTS): Skip if version marker exists
 *                             and version matches. This is the default.
 *                           - NVRTC_INSTALL_HEADERS_FORCE_OVERWRITE: Clear directory
 *                             first, then install. Guarantees consistency.
 *                           - NVRTC_INSTALL_HEADERS_NO_WAIT: Return NVRTC_ERROR_BUSY
 *                             immediately if lock cannot be acquired.
 *                           NO_WAIT can be combined with FORCE_OVERWRITE.
 * \param   [out] errorLog   Optional pointer to receive detailed error message on failure.
 *                           If non-NULL, \p *errorLog will be set to point to a string
 *                           describing the error cause. Note: subsequent API calls from
 *                           the same thread may overwrite this message.
 *                           May be NULL if error details are not needed.
 * \return
 *   - \link #nvrtcResult NVRTC_SUCCESS \endlink
 *   - \link #nvrtcResult NVRTC_ERROR_INVALID_INPUT \endlink (invalid path or
 *     conflicting flags like SKIP_IF_EXISTS | FORCE_OVERWRITE)
 *   - \link #nvrtcResult NVRTC_ERROR_BUILTIN_OPERATION_FAILURE \endlink
 *     (extraction failed or version mismatch)
 *   - \link #nvrtcResult NVRTC_ERROR_BUSY \endlink (lock held by another process
 *     and NVRTC_INSTALL_HEADERS_NO_WAIT was specified)
 *
 * \see nvrtcCompileProgram
 */
nvrtcResult nvrtcInstallBundledHeaders(const char* installPath, unsigned int flags,
                                       const char** errorLog);
 ...
 */
typedef struct {
  int available;              /**< Non-zero if bundled headers are available */
  size_t compressedSize;      /**< Size of compressed archive in bytes */
  size_t uncompressedSize;    /**< Estimated size when extracted in bytes */
  int cudaVersionMajor;       /**< CUDA major version of bundled headers */
  int cudaVersionMinor;       /**< CUDA minor version of bundled headers */
  unsigned int numFiles;      /**< Number of header files in the bundle */
} nvrtcBundledHeadersInfo;

/**
 * \ingroup bundled_headers
 * \brief   nvrtcGetBundledHeadersInfo queries information about the bundled
 *          headers without extracting them.
 *
 * This function allows users to determine if bundled headers are available
 * and get size estimates before calling nvrtcInstallBundledHeaders.
 *
 * \param   [out] info Pointer to structure to receive header information.
 * \param   [out] errorLog Optional pointer to receive detailed error message on failure.
 *                         If non-NULL, \p *errorLog will be set to point to a string
 *                         describing the error cause. Note: subsequent API calls from
 *                         the same thread may overwrite this message.
 ...
 * \return
 *   - \link #nvrtcResult NVRTC_SUCCESS \endlink
 *   - \link #nvrtcResult NVRTC_ERROR_INVALID_INPUT \endlink (info is NULL)
 *   - \link #nvrtcResult NVRTC_ERROR_BUILTIN_OPERATION_FAILURE \endlink
 *     (failed to query bundled headers)
 */
...
/**
 * \ingroup bundled_headers
 * \brief   nvrtcRemoveBundledHeaders removes previously installed bundled headers.
 *
 * This function removes the headers installed by nvrtcInstallBundledHeaders,
 * helping users manage disk space. It recursively removes all files and
 * subdirectories within the installation directory.
 *
 * \note    This function will remove ALL contents of the specified directory,
 *          not just files installed by NVRTC. Use with caution.
 *
 * \param   [in] installPath Path where headers were previously installed.
 *                           Must be the same path used with nvrtcInstallBundledHeaders.
 * \param   [out] errorLog Optional pointer to receive detailed error message on failure.
 *                         If non-NULL, \p *errorLog will be set to point to a string
 *                         describing the error cause. Note: subsequent API calls from
 *                         the same thread may overwrite this message.
 ...
 * \return
 *   - \link #nvrtcResult NVRTC_SUCCESS \endlink
 *   - \link #nvrtcResult NVRTC_ERROR_INVALID_INPUT \endlink (invalid path)
 *   - \link #nvrtcResult NVRTC_ERROR_BUILTIN_OPERATION_FAILURE \endlink
 *     (removal failed)
 */
 ...
 *
 *   - Tile compilation
 *     - \c --enable-tile (\c -enable-tile)
 * 
 *       Enable support for Tile constructs (e.g. \c __tile__ ) and define the macro \c __CUDACC_TILE__ .
 *     - \c --tile-only (\c -tile-only)
 *
 *       Enable support for parsing Tile constructs and define the macro \c __CUDACC_TILE__, but omit code generation for   non-Tile code (e.g. \c __global__ function). 
 *     - \c --simt-only (\c -simt-only)
 *    
 *        Enable support for parsing Tile constructs and define the macro \c __CUDACC_TILE__, but omit code generation for Tile code (e.g. \c __tile_global__ function). 
 *     - \c --default-tile  (\c --default-tile)
 *
 *       Consider an unannotated function or static storage duration variable as having an implicit \c __tile__ annotation. If the static storage duration variable violates Tile restrictions (e.g. cannot be of pointer type), then the implicit annotation is silently omitted by default; use \c -diagnose-implicit-tile-var to enable compiler diagnostics for such cases.
 *
 *       \c -default-tile can be used in conjunction with \c -default-device. If both flags are specified, the unannotated function and/or static storage duration variable will be considered to have both \c __tile__ and \c __device__ implict annotations. There will be a separate copy of the function/variable in the generated SIMT and cuda_tile programs, at present.
 *
 *     - \c --diagnose-implicit-tile-var (\c -diagnose-implicit-tile-var)
 *
 *       When combined with \c -default-tile, diagnose static storage duration variables that could not be implicitly annotated with the \c __tile__ memory space specifier (e.g., due to pointer type restrictions).
 *   - Debugging support
 *     - \c --device-debug (\c -G)
 * 
 ...
 * 
 *     - \c --use-bundled-headers=\<dir\>
 *
 *       Install bundled CUDA headers to \c \<dir\> and add include paths.
 *       This is a convenience flag that combines calling
 *       ::nvrtcInstallBundledHeaders and adding \c -I\<dir\> and
 *       \c -I\<dir\>/cccl to the include search path.  Headers are installed
 *       only if they don't already exist at the specified location.
 *
