...
// Non-static: libmpv_vk.c references this to register its mpvk_ctx
const struct ra_swapchain_fns vulkan_swapchain;

// Forward declaration for libmpv vulkan backend check
extern const struct ra_swapchain_fns libmpv_vk_swapchain_fns;
...
struct mpvk_ctx *ra_vk_ctx_get(struct ra_ctx *ctx)
{
    ...
    // libmpv vulkan backend: swapchain priv's first field is mpvk_ctx*
    // Go through void* because swapchain->priv is typed as the local
    // struct priv, not the libmpv_vk priv struct.
    if (ctx->swapchain && ctx->swapchain->fns == &libmpv_vk_swapchain_fns) {
        void *raw = (void *)ctx->swapchain->priv;
        return *(struct mpvk_ctx **)raw;
    }
    return NULL;
}
...
void ra_vk_ctx_uninit(struct ra_ctx *ctx)
{
    ...
    // libmpv vulkan backend handles its own cleanup — skip
    if (ctx->swapchain->fns == &libmpv_vk_swapchain_fns)
        return;
