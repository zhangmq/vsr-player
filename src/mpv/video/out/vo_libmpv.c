...
int mpv_render_context_render(mpv_render_context *ctx, mpv_render_param *params)
{
    ...
    // Record RT dimensions even when SKIP_RENDERING — needed by
    // VOCTRL_GET_RENDER_TARGET_SIZE for adaptive VSR scale on the first frame.
    int vp_w, vp_h;
    int rt_err = ctx->renderer->fns->get_target_size(ctx->renderer, params,
                                                      &vp_w, &vp_h);
    if (rt_err < 0) {
        mp_mutex_unlock(&ctx->lock);
        return rt_err;
    }
    bool rt_changed = (ctx->vp_w != vp_w || ctx->vp_h != vp_h);
    if (rt_changed) {
        ctx->vp_w = vp_w;
        ctx->vp_h = vp_h;
        if (ctx->vo)
            MP_VERBOSE(ctx->vo, "libmpv: RT recorded = %dx%d (img=%dx%d)\n",
                       ctx->vp_w, abs(ctx->vp_h),
                       ctx->img_params.w, ctx->img_params.h);
    }

    if (do_render) {
        if (ctx->vo && (rt_changed || ctx->need_resize))
        {
...
static void flip_page(struct vo *vo)
{
    ...
    while (ctx->next_frame) {
        if (mp_cond_timedwait_until(&ctx->video_wait, &ctx->lock, until)) {
            if (ctx->next_frame) {
                // 每帧等待超时诊断（per-frame）→ DBG，非中间诊断
                MP_DBG(vo, "mpv_render_context_render() not being called "
                           "or stuck.\n");
                goto done;
            }
        }
    ...
    while (ctx->expected_flip_count > ctx->flip_count) {
        ...
        if (!ctx->flip_count)
            break;
        if (mp_cond_timedwait_until(&ctx->video_wait, &ctx->lock, until)) {
            MP_DBG(vo, "mpv_render_report_swap() not being called.\n");
            goto done;
        }
    }
...
static int control(struct vo *vo, uint32_t request, void *data)
{
    ...
    case VOCTRL_UPDATE_RENDER_OPTS:
        ...
        vo->want_redraw = true;
        return VO_TRUE;
    case VOCTRL_GET_RENDER_TARGET_SIZE: {
        int res_w = ctx->vp_w;
        int res_h = abs(ctx->vp_h);
        // VO 每帧查询渲染目标尺寸（per-frame）→ DBG
        MP_DBG(vo, "libmpv: VOCTRL_GET_RENDER_TARGET_SIZE → %dx%d (%s)\n",
                   res_w, res_h, (res_w > 0 && res_h > 0) ? "ok" : "unavailable");
        if (res_w > 0 && res_h > 0) {
            ((int *)data)[0] = res_w;
            ((int *)data)[1] = res_h;
            return VO_TRUE;
...
static int reconfig(struct vo *vo, struct mp_image_params *params)
{
    ...
    MP_VERBOSE(vo, "libmpv: reconfig — img_params %dx%d p%d:%d fmt=%s(%d) "
               "hw=%s(%d) repr.sys=%d levels=%d\n",
               params->w, params->h, params->p_w, params->p_h,
               mp_imgfmt_to_name(params->imgfmt), params->imgfmt,
               mp_imgfmt_to_name(params->hw_subfmt), params->hw_subfmt,
               params->repr.sys, params->repr.levels);
