...
static void cuda_ext_vk_uninit(const struct ra_hwdec_mapper *mapper, int n)
{
    ...
    // 销毁前同步 stream 0：在途 map（cuMemcpy2DAsync + external semaphore
    // 操作，Vulkan 路径无 do_full_sync，见 hwdec_cuda.c mapper_map）可能
    // 仍引用下面销毁的 semaphore/memory——不等就销毁会让 GPU 在途工作
    // 引用已销毁资源，驱动层 undefined → GPU hang（Xid 109 CTX SWITCH
    // TIMEOUT）。原生 mpv VO 的 reconfig 发生在帧渲染完成后，本项目的
    // vo_libmpv + Qt untimed 渲染下 reconfig（全屏/窗口切换）可能撞上
    // 在途 map，触发该竞态。与 hwdec_cuda_gl.c 的 do_full_sync 对齐。
    //（在途 map 的等待由 ext_wait 的 flush 满足——见 cuda_ext_vk_wait，
    // 因此此同步不会死锁。）
    cu->cuStreamSynchronize(0);
...
static bool cuda_ext_vk_wait(const struct ra_hwdec_mapper *mapper, int n)
{
    ...
    // 立即提交 hold 的 signal：libplacebo 将 hold 的 semaphore signal
    // 排队到"下一次提交"才执行——若不 flush，本帧 map 的 CUDA 等待
    //（cuWaitExternalSemaphoresAsync）的满足依赖下一次渲染提交；当
    // 渲染停摆（如 mpv 核心线程阻塞在 VSR 引擎重建的
    // cuCtxSynchronize，等 stream 0 上的这个等待）时形成循环死锁，
    // GPU 上下文切换超时 → Xid 109（core dump 实测：vsr_destroy 卡在
    // ctx sync，GUI 线程空闲无提交）。
    // 正确性：hold = "image 可供外部使用"——上次渲染已完成（每帧
    // vkQueueWaitIdle），Vulkan 不再使用该纹理，立即 signal 安全。
    pl_gpu_flush(ra_pl_get(mapper->ra));
