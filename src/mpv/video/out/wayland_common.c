...
int vo_wayland_control(struct vo *vo, int *events, int request, void *arg)
{
    ...
    case VOCTRL_GET_RENDER_TARGET_SIZE: {
        // geometry is the actual rendering size: window size when
        // windowed, display size when fullscreen (window_size freezes
        // under locked_size). Windowed: geometry == window_size.
        MP_VERBOSE(vo, "wayland: VOCTRL_GET_RENDER_TARGET_SIZE → %dx%d (%s)\n",
                   wl->geometry.x1, wl->geometry.y1,
                   (wl->geometry.x1 > 0 && wl->geometry.y1 > 0) ? "ok" : "unavailable");
        if (!wl->geometry.x1 || !wl->geometry.y1)
            return VO_NOTAVAIL;
        ((int *)arg)[0] = wl->geometry.x1;
        ((int *)arg)[1] = wl->geometry.y1;
        return VO_TRUE;
    }
