...
static void write_msg_to_buffers(struct mp_log *log, int lev, bstr text)
{
    ...
    for (int n = 0; n < root->num_buffers; n++) {
        ...
        // vsr-player patch: allow MSGL_STATUS into log buffers. Upstream
        // excludes it (terminal progress-line noise); our libmpv client
        // requests level "status" to receive structured status lines
        // (vf_rife's "fruc-status:") — the request level is the filter.
        if (lev <= buffer_level) {
...
void mp_msg_va(struct mp_log *log, int lev, const char *format, va_list va)
{
    ...
    if (lev == MSGL_STATS) {
        ...
        // vsr-player patch: discard only the terminal output. libmpv
        // clients (no terminal) request status-level messages explicitly
        // and must still receive them via log buffers (vf_rife's
        // "fruc-status:" status lines).
        if (root->num_buffers > 0)
            write_msg_to_buffers(log, lev, root->buffer);
    } else {
