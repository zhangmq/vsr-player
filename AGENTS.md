...
>
> **本文件是项目的唯一权威指令**。原 CLAUDE.md 含未同步的过时内容与幻觉断言历史（modeset、Wayland embedding 两处已被证伪），删除后不再有冲突源；引用任何技术事实前，仍先对照代码验证（见"铁律 #2"）。
...
1. **不主动提交 git**——用户明确说"提交"才提交（2026-08-13 强调）。
2. **证据优先**——任何结论必须有可追溯证据（代码/日志/测试/源码行号）；没有证据就说"不确定"。被用户挑战时第一反应是找证据，不是辩护。搜索不加年份限定，看页面日期判断时效。
3. **结论先验证再引用**——原 CLAUDE.md 是历代会话累积的文档，含未验证断言（modeset、Wayland embedding 两处已被证伪修正），已删除；本文件同样可能过时。引用任何技术事实前对照代码 + 系统实测。
4. **严格对齐 mpv 参考实现**——改任何逻辑前先读 `third_party/mpv/` 对应源码，列出差异再改；不自行设计、不加"优化"。mpv 没有的东西不加（除非用户明确要求）。
5. **不用 QTimer 解决渲染/事件架构问题**——业务触发器不解决架构问题；唤醒/驱动走框架生命周期钩子。
6. **字体样式冻结**——不主动改 font family/size/bold，除非用户点名。
7. **不自证性能瓶颈**——默认硬件性能足够；用户说性能受限才考虑。
8. **日志纪律**——默认只输出 info 及以上（`all=info`）；级别语义对齐 mpv（INFO=状态变更/VERBOSE=低频诊断/DBG=帧级）；调试用 `--msg-level all=v`（帧级 `all=dbg`），benchmark 用 `all=no`。**不要用 `log-file`**（下限 MSGL_DEBUG 会绕过 msg-level 刷屏）。
9. **自取证调试**——需要日志自己用 `--rpc --rpc-socket` 后台驱动播放器抓（stderr 重定向），不指使用户；只在需要目检画面时才找用户。
10. **测试只收集报告**——跑测试后只汇报结果；失败就停下呈现给用户，不自动修。
11. **无 sudo**——需要特权操作标记为需用户执行并给出具体命令。
12. **视觉瑕疵先排除编码**——任何"涂抹/色块"先出 PNG 帧序列/444 编码对照，再归因模型/引擎（yuv420p 色度下采样曾致 3 个引擎全被误判）。
13. **算法单一事实源**——同样的计算逻辑（如 adaptive scale）不得在多个组件重复实现，防止分叉。
...
- **闪回问题已解决**（`vf_rife.c:622-694`）：整数倍率 ≥2 用 vs-mlrt `Interleave([src, interp])` 语义——源帧总保留、插值帧在相对时间点 t=i/k；PTS 打在绝对网格（对齐 minterpolate `out_pts++`/vf_vapoursynth 均匀累积）；非整数倍率保留绝对网格。根因：绝对网格在 VFR 源（goose 33.0/34.0ms 交替 PTS）上使 tval 摆到 ~0.01–0.99 → 极端 tval 插值 + 源帧被吞 → 内容抖动。极端 tval（<0.05/>0.95）复制近端点。
...
- 解码规则：AV1/H.264/HEVC 一律 native decoder + `*_nvdec` hwaccel（get_format）；**永不用 `*_cuvid`**（surface 管理 bug → 周期重复帧）
...
- `README.md`/`README_zh.md`（功能/构建/分发/CLI）、`docs/`（设计记录与参考索引）、`BUG-FIXING-DISCIPLINE.md`（调试守则：证据优先/gdb/git log/diff/二分法）
