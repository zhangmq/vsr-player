...
| 文件 | 职责 |
|------|------|
| `video/filter/vf_vsr.c` | **项目核心**：VSR mpv filter。`mp_filter_info` name="vsr"（L1071）；`vsr_command` 热更新 scale/quality/denoise（L944，不重建 filter 链）；自定义 `m_option_type` "Scale"（L146，mpv 0.41 无 OPT_CHOICE_OR_FLOAT，支持 off/auto/数字/ratio） |
| `video/filter/vsr_proc.c` | VSRProcessor 封装（CUDA + VFX SDK 推理管线） |
| `video/filter/yuv_to_rgba.c/.h` | YUV→RGBA 转换（CPU avfilter graph，scale_cuda 不可用） |
...
## 3. vf_vsr 内部结构（VFX SDK 集成点）

```
demux → decode → [vf_vsr] → VO (libmpv)
                    ↑
    vsr_proc.c: CUDA 上下文 + VFX SDK VSR pipeline
    yuv_to_rgba.c: NV12/P010 → RGBA（scale_cuda 在本机不可用）
```

- 输入：`mp_image`（HW 帧解码后仍在显存）
- 输出：超分后的 `mp_image`（送回 mpv 管线，VO 渲染）
